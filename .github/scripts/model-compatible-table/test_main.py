...
def test_parse_xml():
    ...
    for model_name, tests in model_tests.items():
        ...
        for test in tests:
            ...
            assert test["card"] in all_valid_cards, f"Invalid card for {model_name}: {test['card']}"

            # Validate file path
            assert test["file_path"].endswith(".py"), f"Invalid file path for {model_name}: {test['file_path']}"
