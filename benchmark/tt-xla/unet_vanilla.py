...
from third_party.tt_forge_models.unet.pytorch.src.unet import UNET
from .utils import (
...
# tt-metal vanilla UNet uses 480x640 input size
INPUT_SIZE = [
    (480, 640),
]
...
# tt-metal configuration: [32, 64, 128, 256]
FEATURE_SIZES = [
    [32, 64, 128, 256],
]


@pytest.mark.parametrize("feature_sizes", FEATURE_SIZES, ids=[f"features={item}" for item in FEATURE_SIZES])
@pytest.mark.parametrize("input_size", INPUT_SIZE, ids=[f"input_size={item}" for item in INPUT_SIZE])
@pytest.mark.parametrize("batch_size", BATCH_SIZE, ids=[f"batch_size={item}" for item in BATCH_SIZE])
@pytest.mark.parametrize("loop_count", LOOP_COUNT, ids=[f"loop_count={item}" for item in LOOP_COUNT])
@pytest.mark.parametrize("channel_size", CHANNEL_SIZE, ids=[f"channel_size={item}" for item in CHANNEL_SIZE])
@pytest.mark.parametrize("data_format", DATA_FORMAT, ids=[f"data_format={item}" for item in DATA_FORMAT])
def test_unet_vanilla_torch_xla(
    training,
    batch_size,
    input_size,
    channel_size,
    ...
    """
    This function creates a Vanilla UNet model using PyTorch and torch-xla.
    Matches tt-metal vanilla UNet configuration with 480x640 input size.
    """
    ...
    full_model_name = "Vanilla UNet Torch-XLA (tt-metal config)"
    model_type = "Segmentation, Random Input Data"
    dataset_name = f"Vanilla UNet {input_size[0]}x{input_size[1]}, Random Data"
    num_layers = -1
    ...
    print_benchmark_results(
        model_title=f"Vanilla UNet Torch-XLA ({input_size[0]}x{input_size[1]})",
...
def benchmark(config: dict):
    """
    Run the vanilla UNet torch-xla benchmark.
    Matches tt-metal vanilla UNet configuration.
    """
    ...
    feature_sizes = FEATURE_SIZES[0]
    model_name = config["model"]
    measure_cpu = config["measure_cpu"]

    return test_unet_vanilla_torch_xla(
        training=training,
        batch_size=batch_size,
        input_size=input_size,
        channel_size=channel_size,
