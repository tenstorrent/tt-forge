...
This directory contains reference output files (`.refpt`) used for token accuracy testing in tt-forge LLM benchmarks.
...
4. TOP1 and TOP5 accuracy are computed by comparing predictions against `top5_tokens`

**TOP1 Accuracy**: Percentage where predicted token matches the first entry in top5_tokens
**TOP5 Accuracy**: Percentage where predicted token is in any of the top 5
...
```bash
# Single model
pytest -svv benchmark/tt-xla/llms.py::test_llama_3_2_1b_accuracy --output-file results.json

# All accuracy tests
pytest -svv benchmark/tt-xla/llms.py -k "accuracy" --output-file results.json
```
