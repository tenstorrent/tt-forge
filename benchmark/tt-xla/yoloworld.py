...
from third_party.tt_forge_models.yoloworld.pytorch.loader import ModelLoader as YOLOWorldLoader
from .utils import (
...
def test_yoloworld_torch_xla(
    training, batch_size, input_size, channel_size, loop_count, task, data_format, model_name, measure_cpu
):
    """
    This function creates a YOLO-World model using PyTorch and torch-xla.
    ...
    yoloworld_loader = YOLOWorldLoader()
    model_info = yoloworld_loader.get_model_info().name
    if data_format == "bfloat16":
        framework_model: nn.Module = yoloworld_loader.load_model(dtype_override=torch.bfloat16)
    else:
        framework_model: nn.Module = yoloworld_loader.load_model()

    # Wrap the model to handle YOLO-World's tuple output
    ...
    full_model_name = "YOLO-World Torch-XLA"
    ...
    print_benchmark_results(
        model_title="YOLO-World Torch-XLA",
...
def benchmark(config: dict):
    """
    Run the yoloworld torch-xla benchmark.
    ...
    return test_yoloworld_torch_xla(
        training=training,
        batch_size=batch_size,
        input_size=input_size,
        channel_size=channel_size,
