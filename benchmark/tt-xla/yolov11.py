...
from third_party.tt_forge_models.yolov11.pytorch.loader import (
    ModelLoader as YOLOv11Loader,
    ModelVariant as YOLOv11Variant,
)
from .utils import (
...
def test_yolov11_torch_xla(
    ...
    yolov11_loader = YOLOv11Loader(YOLOv11Variant.YOLOV11N)
    model_info = yolov11_loader.get_model_info().name
    if data_format == "bfloat16":
        framework_model: nn.Module = yolov11_loader.load_model(dtype_override=torch.bfloat16)
    else:
        framework_model: nn.Module = yolov11_loader.load_model()

    # Wrap the model to handle YOLOv11's tuple output
    ...
    full_model_name = "YOLOv11 Torch-XLA"
...
def benchmark(config: dict):
    """
    Run the yolov11 torch-xla benchmark.
